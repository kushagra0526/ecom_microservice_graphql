...
const TIMEOUT = 3000; // 3s — fail fast if a downstream service is down
...
exports.createOrder = async (req, res) => {
  try {
    ...
    // Validate product and user exist before creating the order
    try {
      await Promise.all([
      ...
      // Downstream service unreachable (ECONNREFUSED, ETIMEDOUT, etc.)
      if (!err.response) {
        return res.status(503).json({ message: 'A required service is unreachable. Please try again later.' });
      }
      // 404 from user or product service — entity doesn't exist
