        ...
        <div className="max-w-2xl mx-auto px-6 py-10">
            <div className="flex items-baseline justify-between mb-8">
                <h1 className="text-2xl font-semibold"
                    style={{ fontFamily: 'var(--font-space), system-ui', color: 'var(--ink)' }}>Cart</h1>
                <button onClick={clear} className="text-xs" style={{ color: 'var(--slate)' }}>Clear all</button>
            </div>

            <div className="space-y-3 mb-8">
                {items.map((item) => (
                    <div key={item.id} className="flex items-center gap-3 px-3 sm:px-4 py-3 rounded-xl"
                        style={{ background: 'white', border: '1px solid var(--wire)' }}>
                        <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center flex-shrink-0 text-base"
                            style={{ background: 'var(--wire)' }}>⚡</div>

                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium truncate" style={{ color: 'var(--ink)' }}>{item.name}</p>
                            <p className="text-xs mt-0.5" style={{ fontFamily: 'var(--font-mono), monospace', color: 'var(--signal)' }}>
                        ...
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                            <button onClick={() => updateQty(item.id, item.quantity - 1)}
                                ...
                                style={{ background: 'var(--wire)', color: 'var(--ink)' }} aria-label="Decrease quantity">−</button>
                            ...
                            <button onClick={() => updateQty(item.id, item.quantity + 1)}
                                className="w-7 h-7 rounded-md text-sm font-medium flex items-center justify-center"
                                style={{ background: 'var(--wire)', color: 'var(--ink)' }} aria-label="Increase quantity">+</button>
                        </div>

                        <span className="hidden sm:inline text-sm font-medium w-16 text-right flex-shrink-0"
                            style={{ fontFamily: 'var(--font-mono), monospace', color: 'var(--ink)' }}>
                        ...
                        <button onClick={() => remove(item.id)}
                            ...
                            aria-label="Remove item" style={{ color: 'var(--slate)' }}>×</button>
                    </div>
                ))}
            </div>

            <div className="px-5 py-4 rounded-xl" style={{ background: 'white', border: '1px solid var(--wire)' }}>
